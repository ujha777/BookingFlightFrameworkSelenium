...
public class StandAloneTest2 {
	...
	public static void main(String[] args) throws InterruptedException, AWTException {
		...
		WebElement destination = driver.findElement(By.xpath("//label[@id=':R1l36dalqmt2mmH1:']"));
		Actions actions = new Actions(driver);
		actions.moveToElement(destination).click().build().perform();
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(20));
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//li[@id='listbox-option-1']//span[@class='d-10pcgf6']")));
		WebElement autoSuggestion = driver.findElement(By.xpath("//li[@id='listbox-option-1']//span[@class='d-10pcgf6']"));
		...
		try {
			for (WebElement currentdates : dates) {
				if (currentdates.getText().equals(String.valueOf(fromDate)) || currentdates.getText().equals(String.valueOf(toDates))) {
					currentdates.click();
				}
			}
		...
		List<WebElement> searchResults = driver.findElements(By.xpath("//div[@class='sc-928ccd0e-0 dAlzUa']/div/a[2]/div[1]/div[1]"));
